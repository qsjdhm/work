...
<script type="text/ecmascript-6">
    ...
    import {
        SET_USERNAME,
        SET_PASSWORD
    } from '../vuex/modules/LoginPage';

	export default {
		computed: {
			// 因为用到了modules，所以正确的变量位置在store.state.LoginPage中
			...mapState({
				username: state => state.LoginPage.username,
...
<style scoped>
    ...
    .wrapper {
        color: white;
        background: #2db7f5;
        background: -webkit-linear-gradient(top left, #2db7f5 0%, #2db7f5 100%);
        background: linear-gradient(to bottom right, #2db7f5 0%, #2db7f5 100%);
        opacity: 0.8;
        position: absolute;
        top: 35%;
        left: 0;
